import { embeddingNodeDataDecoder } from "./embeddingDataDecoder";
import { embeddingNodeDataEncoder } from "./embeddingDataEncoder";
import { createSignal, createEffect } from "solid-js";
import useStateContext from "../../../../../useStateContext";

export default function useEmbeddingNodeParameterState() {
  ...
  const dataInsertHandler = (fieldName: string, data: any) => {
    ...
    if (fieldName in previousData()) {
      if (previousData()[fieldName] === data) {
        ...
        const formatted = embeddingNodeDataEncoder(
          submittedData(),
  ...
  createEffect(() => {
    ...
    if (!triggerKey.has(currentFormConfig().id)) {
      ...
      const decoded = embeddingNodeDataDecoder(data);
      if (decoded) {
